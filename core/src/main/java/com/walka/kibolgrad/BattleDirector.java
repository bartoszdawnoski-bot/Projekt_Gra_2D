...
public class BattleDirector {
    // REFERENCJE
    final private Array<Fighter> FighterRegistry;
    final private ObjectMap<Fighter, Array<Fighter>> FighterMap = new ObjectMap<>();
    ...
    public void assignRole() {
        ...
        for (int j = 0; j < FighterRegistry.size; j++) {
            ...
            Fighter myCommander = null;
            for (Fighter ally : FighterRegistry) {
                if (ally.getTeam() == f.getTeam() && ally.isBoss() && ally.getFighterState() != FighterState.DEAD) {
                    myCommander = ally;
                    break;
                }
            }
            ...
            for (int k = 0; k < FighterRegistry.size; k++) {
                ...
                if (f2 != f && f.getTeam() != f2.getTeam()) {
                    ...
                    if (myCommander != null && myCommander != f) {
                        float distToCommander = f2.getPosition().dst(myCommander.getPosition());

                        if (distToCommander < 200f) {
                            score += 8000f;
                        }
                    }
                    ...
                    if (f2.getStats().range > f.getStats().range) {
                        ...
                        isCurrentTerrified = MathUtils.randomBoolean(0.5f);
                    } else {
            ...
            float attackDist = f.getStats().range * 0.75f;
            attackDist += MathUtils.random(-5f, 5f);
            ...
            if (isTerrified) {
                ...
                if (myCommander != null && myCommander != f) {
                    ...
                    float bodyguardRadius = 80f;
                    orderPoint.x = myCommander.getPosition().x + MathUtils.cos(angle) * bodyguardRadius;
                    orderPoint.y = myCommander.getPosition().y + MathUtils.sin(angle) * bodyguardRadius;
                } else {
                    float angle = (attackers.size * 60f) * MathUtils.degRad;
            ...
            orderPoint.x = MathUtils.clamp(orderPoint.x, 560f, 1330f);
            orderPoint.y = MathUtils.clamp(orderPoint.y, 180f, 850f);
